...
// A client's Stellar::Keypair to be used to sign the challenge transaction.
const CLIENT_KEY_PAIR = stellar.Keypair.fromSecret(
...
request(`${DOMAIN}/.well-known/Stellar.toml`, (err, res, body) => {
  ...
  // Server's Stellar::Keypair used to to check the server's signature.
  const SERVER_KEY_PAIR = stellar.Keypair.fromPublicKey(
    config.WEB_AUTH_ACCOUNT
  );

  // Request the challenge transaction
  request(
    `${ENDPOINT}?public_key=${CLIENT_KEY_PAIR.publicKey()}`,
    { json: true },
    (err, res, body) => {
      if (err) {
        return console.log(err);
      ...
      request.post(
        { url: ENDPOINT, form: { transaction: signed } },
        (err, res, body) => {
          console.log(body);
        }
      );
